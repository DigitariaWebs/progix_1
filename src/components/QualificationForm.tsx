...
// ─── Main Component ───────────────────────────────────────────────────────────
// collectContact=false renders the "direct link" variant sent to known clients:
// no email/phone step, submissions tagged as variant 'direct' for the API.

export default function QualificationForm({ collectContact = true }: { collectContact?: boolean }) {
  const mobile = useBreakpoint();
  const steps = collectContact ? STEPS : STEPS.filter(s => s.kind !== 'contact');
  const totalContent = collectContact ? TOTAL_CONTENT : TOTAL_CONTENT - 1;
  ...
  const progress = step.number != null
    ? ((step.number - 1) / totalContent) * 100
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...formDataRef.current, variant: collectContact ? 'site' : 'direct' }),
      });
  ...
  }, [submitting, collectContact, steps.length]);
  ...
  const isLastContent = step.id === (collectContact ? 'contact' : 'questions');
      ...
      {step.number != null && (
        <div style={{
          ...
          <span style={{ fontSize: mobile ? '12px' : '15px', fontWeight: 500, color: C.textMuted, letterSpacing: '0.04em', lineHeight: 1 }}>
            {String(totalContent).padStart(2, '0')}
          </span>
        </div>
      )}
